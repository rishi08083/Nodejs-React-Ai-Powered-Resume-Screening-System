...
} from "../../api-services/recruiterService"; // Import service functions
...
export default function RecruiterRequests() {
  ...
  const handleAccept = async (email) => {
    try {
      const response = await acceptRecruiterRequest(email); // Call API to accept request
      if (response.ok) {
        setMessage("Request accepted successfully!");
        setRequests(requests.filter((req) => req.email !== email)); // Remove the accepted request from the list
      } else {
  ...
  const handleReject = async (email) => {
    try {
      const response = await rejectRecruiterRequest(email); // Call API to reject request
      if (response.ok) {
        setMessage("Request rejected successfully!");
        setRequests(requests.filter((req) => req.email !== email)); // Remove the rejected request from the list
      } else {
