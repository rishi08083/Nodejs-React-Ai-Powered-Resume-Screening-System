...
const screenCandidate = async (req, res) => {
  try {
    ...
    if (!candidate_id) {
      return res
        .status(400)
        .json({ status: "error", message: "Missing candidate_id" });
    }
    ...
    if (!FastAPI_server_url) {
      return res.json({
        status: "error",
        message: "env FastAPI_server_url not present",
      });
    }
    ...
    const aiResponse = await axios.post(
      `${FastAPI_server_url}/screen_candidates_2`,
      requestBody,
      {
        headers: {
    ...
    const existingScreening = await db.ScreeningResults.findOne({
      where: { candidate_id },
    });
